...
<html lang="pl">
	<head>
		...
		<title>Kalendarz na rok szkolny 2011/2012 | Strona klasy 3c</title>
	...
	<body>
		...
		<div id="page">
			<h2 class="title">Kalendarz na rok szkolny 2011/2012</h2>
			<?php
				$data = date("Ymd");
				include('polacz.php');
				$query = mysql_query("SELECT * FROM `kalendarz` WHERE `sort` >= ".$data." ORDER BY `data`");
				while($rekord = mysql_fetch_array($query)){
					echo '<p class="'.$rekord[4].'"><span style="font-weight:bold;">'.$rekord[1].'</span> '.$rekord[3].'</p>';
				}
			?>
