...
<html lang="pl">
	<head>
		...
		<title>Lista nauczycieli | Strona klasy 3c</title>
	...
	<body>
		...
		<div id="page">
			<h2 class="title">Lista nauczycieli</h2>
				<div style="margin: 25px auto 0; width: 680px;">
					<?php
						include('polacz.php');//dolaczenie pliku laczacego z baza
						$query = mysql_query("select id, imie, nazwisko, przedmiot from nauczyciele");//zapytanie do bazy
						...
						while ($rekord = mysql_fetch_array($query)){//petla wyswietlajaca dane z bazy
							echo '<div class="osoba2"><img alt="'.$rekord[1].' '.$rekord[2].'" src="images/'.$rekord[2].'.jpg"><p class="p1">'.$rekord[1].' '.$rekord[2].'</p><p class="p2">'.$rekord[3].'</p></div>';//szablon dla jednego nauczyciela
						}
						echo '<div style="clear: both;"></div>';//pobrawienie wyswietlania
					?>
				</div>
