...
var Thumbs = {
	...
	load:function(){
		...
		var img;
		for(var i in images){
			img='<img src="'+images[i].thumb.cleanupHTML()+'" /><b> </b>';
	...
	markNewFolders:function(){
		...
		$(".folder .thumb img").each(function(){
			mod=$(this).attr('rel');
			if(mod.length && (typeof (mod=parseInt(mod))!='NaN') && ((days-mod)<=newDays))
...
var Imgs = {
	...
	go:function(n){
		...
		if(ci.video){
			...
			var img=$(new Image());
			img.load(Imgs.loaded(n,img)).attr({src:ci.file.cleanupHTML(),width:ci.width,height:ci.height});
		}
	},
...
$(document).ready(function(){
	...
	//$('#tweetTwitter').append('<a href="http://twitter.com/share" class="twitter-share-button" data-count="none">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>');
