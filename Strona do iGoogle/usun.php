		...
		<div id="page" class="dodaj_usun">
			<?php
				...
				if($_POST){
					$haslo = $_POST['pass'];
					if($haslo == $pass){
						echo '<form method="post"><p><label for="id">Numer zakładki: </label><input type="text" name="id"><input type="hidden" name="pass" value="xxx"></p><p><input type="submit" value="Usuń zakładkę"></p></form>';
						$spr = $_POST['id'];
						if($spr != NULL) {
							if($_SERVER['REQUEST_METHOD'] == 'POST'){
								$query = mysql_query("DELETE FROM `linki` WHERE `id`='".$_POST['id']."'");
								echo '<p>Usunięto zakładkę! <a href="index.php">Wróć</a></p>';
							}
						}
					}
					else {
